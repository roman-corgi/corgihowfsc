"""
Populate the optical model definition file and generate the FITS files for it.

This is for use with efc_computation() from cgi-howfsc to compute DM seeds as a starting
point for corgihowfsc. There is a separate script for creating the model for corgihowfsc.
...
python write_model_for_cgihowfsc.py nfov 1 360deg
python write_model_for_cgihowfsc.py wfov 1 360deg
python write_model_for_cgihowfsc.py wfov 4 360deg
python write_model_for_cgihowfsc.py spec 3 130deg
python write_model_for_cgihowfsc.py spec 2 130deg
python write_model_for_cgihowfsc.py specrot 3 130deg
python write_model_for_cgihowfsc.py specrot 2 130deg
...
from write_cstrat_file_noprobe import write_cstrat_file
...
def full_path(coro_mode):
    ...
    return os.path.join(DM_SEED_PATH, coro_mode)
...
def setup_steps(fn_setup):
    ...
    parent_folder = full_path(setup_dict['coro_mode_parent'])
    any_folder = os.path.join(parent_folder, 'any')
    coro_folder = os.path.join(setup_dict['coro_mode'])
    # parent_folder = full_path('%s_%d' % (setup_dict['mode_type'], extract_first_number(setup_dict['cal_bandpass'])))
    # any_folder = os.path.join(parent_folder, 'any')
    # coro_folder = os.path.join(parent_folder, 
    #                            ('%s_%d_%s' % (setup_dict['mode_type'], extract_first_number(setup_dict['cal_bandpass']), setup_dict['label'])),
    #                            )
    # os.makedirs(parent_folder, exist_ok=True)
    # os.makedirs(any_folder, exist_ok=True)
    os.makedirs(coro_folder, exist_ok=True)
        
    # Copy setup YAML file to coro_mode direction
    shutil.copy(fn_setup, coro_folder)
    ...
    fn_homf = os.path.join(coro_folder, setup_dict['fn_howfsc_yaml'])
    writeyaml(outdict=model_dict, path=fn_homf)

    # Create folders for all the subbands
    for ii in range(setup_dict['n_subband']):

        fn_subband_folder = os.path.join(coro_folder, ('subband%d' % ii))
        isExist = os.path.exists(fn_subband_folder)
        if not isExist:
            os.makedirs(fn_subband_folder)
...
def write_cstrat(fn_setup):
    ...
    fn_cstrat = os.path.join(full_path(coro_mode), f'cstrat_noprobe_{coro_mode}.yaml')
    ...
    if setup_dict['is_hlc']:
        ...
        # nlam = 7
        # nexcam = 81  # ceil_odd(4.0*2*9.7+1)
        ...
        # if 'wfov' in coro_mode or 'mswc' in coro_mode:
        #     nlam = 5
        #     nexcam = 127  # ceil_odd(3*20.4*2+1) = 125
        # elif 'spec' in coro_mode:
        #     nlam = 9
        #     nexcam = 81  #  ceil_odd(4*9.4*2+1) = 77
...
def write_model(model_type, band_num, label):
    ...
    fn_setup = f'./homf_params/homf_params_{model_type}_band{str(band_num)}_{label}.yaml'
